...
rem::code lexer::operator>>(token_info& token)
{
    if (!source.tokens_table)
        throw sys::exception()[ sys::error() << "Tokens reference table must be set prior to call this method. - Aborting the lexer."];
    if (!scanner.has_text())
        throw sys::exception()[ sys::error() << "No text to scan  - Aborting the lexer."];
...
rem::code lexer::parse_quotes(token_info& token)
{
    ...
    if (!r)
    {
        auto l = sys::error(); l << "Failed to scan literal: " << l << scanner.mark() << l;
        return rem::code::rejected;
    }
...
{
    //auto l = sys::debug(); l << scanner.cursor << l;
    auto cc = scanner.cursor;
    cc += token.cursor.length;
    while ((*cc != '\r') && (*cc != '\n') && (cc < scanner.end)) ++cc;
    //l = sys::debug(); l << scanner.mark() << l;
    scanner.cursor = cc;
    scanner.sync();
    return rem::code::accepted;
}
...
rem::code lexer::parse_block_comment(token_info&token)
{
    ...
    if(cc >= scanner.end)
    {
        auto l = sys::error(); l << color::red << "unterminated bloc comment" << l;
        return rem::code::rejected;
    }
    ++cc;
    if (*cc == '/')
...
rem::code lexer::parse_f4ac(token_info& token)
{
    ...
    auto l = sys::debug(); l << "F4AC: " << l << token.to_string() << l;
...
{
    scanner = source.text;
    auto l  = sys::debug(); l << "Parsing text: " << l << source.text << l;
    if (!source.tokens_table)
        throw sys::exception()[ sys::error() << "Tokens reference table must be set prior to call this method. - Aborting the lexer."];
    if (!scanner.has_text())
        throw sys::exception()[ sys::error() << "No text to scan  - Aborting the lexer."];
...
{
    auto l = sys::info(); l << "scanned " << color::lime << source.tokens->size() << color::r << " tokens" << l;
...
rem::code lexer::push(token_info&token)
{
    ...
    scanner.skip_ws();
    // auto l = sys::info(); l << a_token.to_string() << l;
